...
function assemble(parsed, context) {
  ...
  for (const node of parsed.children) {
    const check_if_the_only_argument_is_register = () => {
      ...
      if (
        node.children[0].getRegisterNumber(context.namedRegisters) === "none"
      ) {
        alert(
          "Line #" +
            node.lineNumber +
    ...
    if (/^address$/i.test(node.text))
      ...
      if (node.children.length === 1) {
        if (
          node.children[0].getLabelAddress(
            context.labels,
            context.constants
          ) === "none"
        ) {
        ...
        if (
          node.children[2].getLabelAddress(
            context.labels,
            context.constants
          ) === "none"
        ) {
        ...
        if (/^z$/i.test(node.children[0].text))
          machineCode[address].hex =
            "32" +
            node.children[2].getLabelAddress(context.labels, context.constants);
      ...
      machineCode[address].hex =
        "26" +
        node.children[0].children[0].getRegisterNumber(context.namedRegisters) +
        node.children[0].children[2].getRegisterNumber(context.namedRegisters) +
        "0";
      address++;
