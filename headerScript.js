...
function displayRegistersAndFlags() {
  for (let i = 0; i < 2; i++)
    for (let j = 0; j < 16; j++) {
      ...
      else if (tableCell.innerHTML === "00") tableCell.className = "inactive";
  ...
  if (
    document.getElementById("interrupt_flag").innerHTML !== flagIE.toString() &&
    flagIE
  )
...
function highlightToken(token) {
  ...
  if (/^N?[CZAB]$/i.test(token)) return `<span class="flag">${token}</span>`;
...
function setupLayout() {
  ...
  if (windowHeight < 400) {
    ...
    document.getElementById("UART_IO").style.top = 910 + 2 * 210 + 50 + "px"; // Has no effect if the UART_IO is not shown
...
function displayOutput() {
  ...
  for (let i = 0; i < 8; i++)
    document
      ...
      .setAttribute("fill", output[0] & (1 << i) ? "lightGreen" : "#333333");
}
...
function fetchExample(exampleName) {
  ...
  fetch(
    ...
    .catch((error) =>
      alert("Fetching the example code from GitHub unsuccessful! " + error)
    );
}
