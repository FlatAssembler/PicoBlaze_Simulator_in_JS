/*
    This is a set of integration tests (not unit tests) testing the tokenizer
   and the parser together. Previously, they were written in the
   arrange-and-assert form with all of them having the same logic. Then I asked
   GitHub Copilot to rewrite them all from the arrange-and-assert form into the
...
const list_of_directives = require(
    "../list_of_directives.js"); // ... as well as on the list of directives
                                 // (mnemonics and preprocessor directives).
...
const cases = [
  ...
  [
    "Instruction with two operands", "load s0, s1",
    '("assembly" ("load" "s0" "," "s1"))'
  ],
  [
...
`,
  ...
  [
    "Ternary conditional operator", "2+2<5?1:0",
    '("assembly" ("?:" ("<" ("+" "2" "2") "5") "1" "0"))'
  ],
];
