...
describe("PicoBlaze MachineCode Simulator", () => {
  ...
  test("sub 0 - 1 equals 0xff", () => {
    // This once crashed the simulator:
    // https://github.com/FlatAssembler/PicoBlaze_Simulator_in_JS/pull/15#issuecomment-1696714640
    global.machineCode = [
      {hex : "01000", line : 3},
      {hex : "19001", line : 4},
    ];
  ...
  test("jump nz + labels work", () => {
    ...
    for (let i = 0; i < 255 * 3; i++) {
      // we do it a few times since it's fast :). took less than 100ms on my
      // machine
  ...
  test("Input UART to register", () => {
    ...
    global.machineCode = str.split("").map(
        (c, i) =>
            // Input to Hello to s0, s1, ...sN
        ({hex : "09" + i + "03", line : i + 1}),
    );

    global.machineCode.forEach(() => { simulator.simulateOneInstruction(); });
  ...
  test("Output register to UART terminal", () => {
    ...
    const machineCode =
        "Hello".split("")
            .map((c, i) => [
                     ...
                     {
                       hex : "010" + c.charCodeAt(0).toString(16),
                       line : i * 2 + 1
                     },
                     // Write to port 3
    ...
    machineCode.forEach(() => { simulator.simulateOneInstruction(); });
