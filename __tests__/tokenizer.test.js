/*
Hackish way to import TreeNode using a babel plugin to export all without
modifying the actual code. Ideally the project should be using ES modules or a
...
describe("PicoBlaze Tokenizer", () => {
  ...
  test(
      "Labels are tokenized correctly 2",
      () => { // https://github.com/FlatAssembler/PicoBlaze_Simulator_in_JS/issues/31
        const tokens = tokenizer.tokenize(`address 0
      label1:
      load s0, s1
      label2: load s1, 1`);
        expect(tokens.map((t) => t.text)).toEqual([
